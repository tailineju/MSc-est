...
mle_moda_bs <- function(x, t) {
  ...
  loglik_moda <- function(par) {
    ...
    beta_orig <- exp(log_moda_Y) / (1 - phi_param)
    terms <- log(t)

    #pdf da distribuição log-BS
    z <- (sqrt(t/beta_orig) - sqrt(beta_orig/t))/alpha_orig
    pdf_bs <- (1/(alpha_orig*sqrt(t)))*(sqrt(t/beta_orig) + sqrt(beta_orig/t))*dnorm(z)
    pdf_bs[pdf_bs <= 0] <- .Machine$double.xmin #substitui val menor ou igual a 0 por um número muito pequeno
    
    #log-verossimilhança
    l_i <- log(pdf_bs)
    if (any(!is.finite(l_i))) {
      return(.Machine$double.xmax)} #retorna um valor muito grande se algum log for infinito
