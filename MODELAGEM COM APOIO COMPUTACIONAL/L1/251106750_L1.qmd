...
mle_moda_bs <- function(x, t) {
  ...
  loglik_moda <- function(par) {
    ...
    alpha_orig <- sqrt(phi_param) #proposta do artifo
    beta_orig <- exp(log_moda_Y) / (1 - phi_param) #proposta do artigo
    terms <- log(t)

    #pdf da distribuição log-BS
    z <- (sqrt(t/beta_orig) - sqrt(beta_orig/t))/alpha_orig
    pdf_bs <- (1/(alpha_orig*sqrt(t)))*(sqrt(t/beta_orig) + 
    sqrt(beta_orig/t))*dnorm(z)
